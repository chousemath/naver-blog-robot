...
(async () => {
    ...
    if (err != null) {
        console.log('fillLoginPage:', (err as Error).message);
        browser.close();
        process.exit();
    }
    ...
    while (true) {
        ...
        if (err != null) {
            console.log('switchToMyComments:', (err as Error).message);
            browser.close();
            process.exit();
        }
        await sleep(1000);
        ...
        const getMyComments = await page.evaluate((): Array<string> => {
            return Array.from(document.getElementsByClassName('text_news'))
                .filter((a: Element) => (a as HTMLAnchorElement).innerText.indexOf('삭제된 글입니다') === -1)
